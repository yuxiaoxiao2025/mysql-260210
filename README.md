...
| 模式 | 命令 | 说明 |
|------|------|------|
| **ReACT** | `react` | 推荐，基于工具调用的智能对话 |
| **Chat** | `chat` | 多智能体架构，支持复杂业务操作 |
| **Legacy** | `--agent-mode` | 已废弃，请使用上述模式 |
...
[ReACT] > 查询车牌沪ABC1234的入场记录
[ReACT] > 下发车牌沪ABC1234到国际商务中心
```
...
基于 Reasoning + Acting 的智能对话架构，通过工具调用实现数据库操作。

```
用户输入 → LLM推理 → 工具调用 → 执行 → 响应
              ↑           ↓
           迭代      tool_results
```
...
- `search_schema` - 搜索表结构
- `execute_sql` - 执行 SQL
- `list_operations` - 列出预定义操作
- `execute_operation` - 执行预定义操作
...
```
IntentAgent → RetrievalAgent → SecurityAgent → PreviewAgent → ExecutionAgent
```

**智能体职责：**
| Agent | 职责 |
|-------|------|
| IntentAgent | 识别意图、提取参数 |
| RetrievalAgent | 检索相关 Schema |
| KnowledgeAgent | 知识问答、流式输出 |
| SecurityAgent | SQL 安全验证 |
| PreviewAgent | 变更预览 |
| ReviewAgent | 危险操作确认 |
| ExecutionAgent | 执行操作 |
...
pipeline = RetrievalPipeline(budget_ms=1000)
